    ...
    foreach($_POST['checkBoxArray'] as $postId){
        ...
        switch($bulk_options){
            case 'publish':
                ...
                $update_to_publish = mysqli_query($link, $query);
                break;

            case 'draft':
                $query = "UPDATE posts SET post_status = '{$bulk_options}' WHERE post_id = {$postId} ";
                $update_to_draft = mysqli_query($link, $query);
                break;
            case 'delete':
                $query = "delete FROM posts WHERE post_id = {$postId} ";
                $update_to_delete = mysqli_query($link, $query);
                break;
            case 'clone':
                $query = "SELECT * FROM posts WHERE post_id = '{$postId}' ";
                $select_post_by_id = mysqli_query($link,$query);
...
if(isset($_GET['reset'])){
    $query = "UPDATE posts SET post_views_count = 0 WHERE post_id =" . mysqli_real_escape_string($link, $_GET['reset']) ." ";
    $reset_query = mysqli_query($link, $query);
