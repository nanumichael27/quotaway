...
function users_online(){
    ...
    if($count == null){
        mysqli_query($link, "INSERT INTO online_users(session, time) VALUES('$session', '$time')");
    } else {
        mysqli_query($link, "UPDATE online_users SET time = '$time' WHERE session = '$session' ");
    }

    $user_online_query = mysqli_query($link, "SELECT * FROM online_users WHERE time > '$time_out' ");
...
function register_user($username, $firstname, $lastname, $sex, $email, $password){
    global $link;
        ...
        $registration_user_query = mysqli_query($link, $query);
...
function login_user($username, $password){
    ...
    $select_username_query = mysqli_query($link, $query);
    ...
    global $link;
     if(isset($_POST['submit'])){
                ...
                if($cat_title == "" || empty($cat_title)){
                    ...
                    $stmt = mysqli_prepare($link, "INSERT INTO categories(cat_title) VALUE(?)");
...
function category_table(){
    ...
    $select_categories_query = mysqli_query($link,$query);
    while($row = mysqli_fetch_assoc($select_categories_query)){
    $cat_id = $row['cat_id'];
    $cat_title = $row['cat_title'];

    $post_count_query = "SELECT * FROM posts WHERE post_category_id = {$cat_id} ";
    $post_count = mysqli_query($link, $post_count_query);
    ...
    global $link;
     if(isset($_GET['delete'])){
         ...
         if(isset($_SESSION['admin_id'])){
                $the_cat_id = mysqli_real_escape_string($link, $the_cat_id);
    ...
    if(isset($_GET['delete'])){
        ...
        if(isset($_SESSION['user_role'])){
            if($_SESSION['user_role'] == 'admin'){
                $the_user_id = mysqli_real_escape_string($link, $the_user_id);
...
function delete_post() {
    ...
    if(isset($_GET['delete'])){
        ...
        $the_post_id = mysqli_real_escape_string($link, $the_post_id);

        $query = "DELETE FROM posts WHERE post_id = {$the_post_id}";
        $delete_query = mysqli_query($link, $query);
        $query= "DELETE FROM comments WHERE comment_post_id = {$the_post_id}";
        $delete_com_query = mysqli_query($link, $query);
        if($delete_query){
